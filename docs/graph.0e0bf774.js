...
module.exports = "sample-d3-project/countries.041acedc.csv";
...
function ready(datapoints) {
  ...
  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('r', 4).attr('cx', function (d) {
    ...
    d3.select(nodes[i]).attr('fill', 'black'); // "this" is the thing we're hovering over, but if you use a fat arrow, you need to change it to nodes[i]
    d3.select("#name").text(d.country);
    d3.select("#life").text(d.life_expectancy);
    d3.select("#GDP").text(d.gdp_per_capita);
    d3.select("#info").style('display', 'block');
  }) // this is called an event listener. Events are controlled by the browser
...
//# sourceMappingURL=sample-d3-project/graph.0e0bf774.map